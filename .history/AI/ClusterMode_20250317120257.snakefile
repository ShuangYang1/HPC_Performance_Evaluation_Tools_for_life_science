...
singlenode_scores={}
singlenode_scores["MindSPONGE"]=mindsponge_singlenode_score
singlenode_scores["Alphafold3"]=alphafold_singlenode_score

rule all:
    input:
        expand("{pwd}/log/{mode}/{software}/run.done",mode=mode,software=softwares,pwd=pwd)
    run:
        generate_report(softwares,multitasks_count,gpus,cpus,node_exporter_port,nvidia_gpu_exporter_port,server_ip,server_port,partition,nodelist,singlenode_scores,mindsponge_env,alphafold_path)

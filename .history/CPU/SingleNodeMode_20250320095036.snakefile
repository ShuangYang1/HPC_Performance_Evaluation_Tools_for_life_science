...
import os
from modules.SingleNodeMode.make_sbatch import make_singletask,make_singletask_repeat,make_multitasks
...
rule bwa_repeat:
    ...
    run:
        ...
        threads=cal_singletask_scale(cpus,params.software)
        find_best_singletask_threads(software, threads)
        make_singletask_repeat(partition=partition,nodes=1,ntasks=1,cpus=cpus,cpus_per_task=cpus,software="BWA",pwd=pwd,nodelist=node,repeat=repeat,script_path=singletask_repeat_script_path)
...
rule spades_repeat:
    ...
    run:
        ...
        if not os.path.exists(script_path):
            os.makedirs(script_path)
        singletask_repeat_script_path=f"{script_path}/singletask_repeat.sh"
        make_singletask_repeat(partition=partition,nodes=1,ntasks=1,cpus=cpus,cpus_per_task=cpus,software="SPAdes",pwd=pwd,nodelist=node,repeat=repeat,script_path=singletask_repeat_script_path)
        run_slurm_job(f"sbatch {singletask_repeat_script_path}")
        if delete:
            os.system(f"rm -rf {pwd}/result/SingleNodeMode/{params.software}/singletask/*")
        else:
...
rule bismark_repeat:
    ...
    run:
        ...
        if not os.path.exists(script_path):
            os.makedirs(script_path)
        singletask_repeat_script_path=f"{script_path}/singletask_repeat.sh"
        make_singletask_repeat(partition=partition,nodes=1,ntasks=1,cpus=cpus,cpus_per_task=cpus,software="Bismark",pwd=pwd,nodelist=node,repeat=repeat,script_path=singletask_repeat_script_path)
        run_slurm_job(f"sbatch {singletask_repeat_script_path}")
        if delete:
            os.system(f"rm -rf {pwd}/result/SingleNodeMode/{params.software}/singletask/*")
        else:

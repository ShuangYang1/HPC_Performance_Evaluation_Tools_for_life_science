...
def sponge_report(
    ...
    eff=round(total_score/len(nodelist)/singlenode_scores[software],2)
    ...
    content += "3. 集群综合扩展效率。  \n"
    content += "集群综合扩展效率指标旨在定量评估在单节点相同配置基础上扩展成集群后，整体计算性能相对于单节点性能的提升程度。该指标不仅考虑了任务执行时间的变化，还综合了能耗因素，从而反映出在扩展计算资源时，系统在实际应用中的并行计算效率与资源利用效率之间的平衡。通过对比在单节点和多节点环境下的作业运行时间和能耗数据，该指标为集群调度策略的优化、硬件配置调整以及经济性评估提供了科学、量化的依据。  \n"
    content += "$$\n"
    content += "集群综合扩展效率=\\frac{集群大规模并行计算得分}{单节点性能得分\\times节点数量}  \n"
    content += "$$\n"
    content += "&nbsp;\n"
    ...
    content += f"#### 集群综合扩展效率：{eff}%\n"
    content += "&nbsp;\n"
    return content, total_score,eff
...
def generate_report(
    ...
    if "SPONGE" in softwares:
        content, sponge_score,sponge_eff = sponge_report(
            content,
            multitasks_count,
            gpus,
            cpus,
        ...
        effs.append(sponge_eff)
    if "GROMACS" in softwares:
        content, gromacs_score,gromacs_eff = gromacs_report(
            content,
            multitasks_count,
            gpus,
            cpus,
        ...
        effs.append(gromacs_eff)
    if "AMBER" in softwares:
        content, amber_score,amber_eff = amber_report(
            content,
            multitasks_count,
            gpus,
            cpus,
        ...
        effs.append(amber_eff)
    if "DSDP" in softwares:
        content, dsdp_score,dsdp_eff = dsdp_report(
            content,
            multitasks_count,
            gpus,
            cpus,
    ...
    content += "&nbsp;\n"
    content += f"#### 集群综合扩展效率：{round(sum(effs)/, 2)}\n"
    content += "#### 集群测试环境：\n"

...
rule all:
    ...
    run:
        generate_report(softwares,multitasks_count,repeat,gpus,cpus,node_exporter_port,nvidia_gpu_exporter_port,server_ip,server_port,partition,node,mindsponge_env,alphafold_path)
...
rule mindsponge:
    ...
    run:
        ...
        make_singletask_repeat(partition=partition,nodes=1,ntasks=1,cpus_per_task=cpus,gpus=1,software="MindSPONGE",cuda_home=mindsponge_cuda_home,conda_path=conda_path,env=mindsponge_env,pwd=pwd,nodelist=node,repeat=repeat,script_path=singletask_repeat_script_path)
        ...
        make_multitasks(partition=partition,nodes=1,ntasks=1,cpus_per_task=cpus_per_task,gpus=1,software="MindSPONGE",cuda_home=mindsponge_cuda_home,conda_path=conda_path,env=mindsponge_env,pwd=pwd,nodelist=node,parallel=gpus,multitasks_count=multitasks_count,script_path=multitasks_script_path)
...
rule alphafold:
    ...
    run:
        ...
        if not os.path.exists(script_path):
            os.makedirs(script_path)
        singletask_repeat_script_path=f"{script_path}/singletask_repeat.sh"
        make_singletask_repeat(partition=partition,nodes=1,ntasks=1,cpus_per_task=cpus,gpus=1,software="Alphafold3",cuda_home=alphafold_cuda_home,conda_path=conda_path,env=alphafold_env,pwd=pwd,nodelist=node,repeat=repeat,script_path=singletask_repeat_script_path,alphafold_path=alphafold_path,alphafold_param_path=alphafold_param_path,alphafold_db_path=alphafold_db_path)
        run_slurm_job(f"sbatch {singletask_repeat_script_path}")
        for count in range(1,multitasks_count+1):
        ...
        multitasks_script_path=f"{script_path}/multitasks.sh"
        cpus_per_task=cpus//gpus
        make_multitasks(partition=partition,nodes=1,ntasks=1,cpus_per_task=cpus_per_task,gpus=1,software="Alphafold3",cuda_home=alphafold_cuda_home,conda_path=conda_path,env=alphafold_env,pwd=pwd,nodelist=node,parallel=gpus,multitasks_count=multitasks_count,script_path=multitasks_script_path,alphafold_path=alphafold_path,alphafold_param_path=alphafold_param_path,alphafold_db_path=alphafold_db_path)
        run_slurm_job(f"sbatch {multitasks_script_path}")
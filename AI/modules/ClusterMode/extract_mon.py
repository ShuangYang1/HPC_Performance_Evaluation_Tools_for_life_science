...
def extract_mon(
    ...
    gpu_query_instance = '\{instance="' + gpu_instance + '"\}'
    ...
    extract_gpu = f"curl -G 'http://{server_ip}:{server_port}/api/v1/query_range?query=nvidia_gpu_duty_cycle{gpu_query_instance}&start={start_timestamp}&end={end_timestamp}&step=1s' > {outdir}/{node}_gpu_{bin_num}.json"
    extract_gpu_mem = f"curl -G 'http://{server_ip}:{server_port}/api/v1/query_range?query=nvidia_gpu_memory_used_bytes{gpu_query_instance}&start={start_timestamp}&end={end_timestamp}&step=1s' > {outdir}/{node}_gpuMem_{bin_num}.json"
...
def auto_extract(
    ...
    for node in nodelist:
        ...
        gpu_instance = f"{node}:{nvidia_gpu_exporter_port}"

...
def hpc_resource_radar(cpu, ram, pfs_recv, pfs_send, power, image_path, datatype):
    ...
    if datatype[1] == "singletask":
        if best_mode == 1:
            best_mode = "单"
        plt.title(
            f"{software} {testmode}{best_mode}线程计算资源使用情况（全程{time}s）",
...
def hpc_resource_radar_split(cpu, ram, pfs_recv, pfs_send, power, image_path, datatype):
    time = len(cpu)
    cpu = reduce_vector_dim(cpu)
    ram = reduce_vector_dim(ram)
    pfs_recv = reduce_vector_dim(pfs_recv)
    ...
    axtop.plot(
        angles,
        list(map(lambda x: x * 20 / max(ram) + 60, ram)),
        "b-",
    ...
    if datatype[1] == "singletask":
        if best_mode == 1:
            best_mode = "单"
        plt.title(
            f"{software} {testmode}{best_mode}线程计算资源使用情况（分层级）（全程{time}s）",
            fontproperties=my_font,
            fontsize=10,
            y=1.05,
        )
...
def extract_and_draw(mondir, bins, cpus, outputfile, split_outputfile, datatype):
    cpu_data, power_data, mem_data, read_data, write_data = (
        np.array([]),
        np.array([]),
        np.array([]),
        np.array([]),

...
def make_multitasks(
    ...
    with open(script_path, "w") as f:
        ...
        if software == "BWA":
            ...
            f.write(
                f"(time bismark --parallel {cpus_per_task} --genome {genome} {fastq} -o {resultdir}/${{SLURM_ARRAY_TASK_ID}}) >& {log_path}\n"
            )
        elif software == "STAR":
            ...
            f.write(
                f"(time STAR --runThreadN {cpus_per_task} --genomeDir {genome} --readFilesIn {fastq} --outFileNamePrefix {resultdir}/${{SLURM_ARRAY_TASK_ID}}/ --runMode alignReads --quantMode TranscriptomeSAM GeneCounts --outSAMtype BAM SortedByCoordinate) >& {log_path}\n"
            )

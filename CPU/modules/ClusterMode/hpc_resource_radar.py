...
def extract_and_draw(
    ...
    length=min(node_cpu.shape[0] for node_cpu in cpu_array)
    cpu_data = np.mean([node_cpu[:length] for node_cpu in cpu_array], axis=0)
    power_data = np.sum([node_power[:length] for node_power in power_array], axis=0)
    mem_data = np.sum([node_mem[:length] for node_mem in mem_array], axis=0)
    read_data = np.sum([node_read[:length] for node_read in read_array], axis=0)
    write_data = np.sum([node_write[:length] for node_write in write_array], axis=0)

...
def hpc_resource_radar(cpu, ram, pfs_recv, pfs_send, power, image_path, datatype):
    ...
    if datatype[1] == "singletask":
        ...
        plt.title(
            f"{software} {testmode}最佳并行模式{best_mode}资源使用情况（全程{time}s）",
            fontproperties=my_font,
            fontsize=10,
            y=1.05,
        )
...
def hpc_resource_radar_gpu(gpu, gpu_mem, image_path, datatype):
    ...
    if datatype[1] == "singletask":
        ...
        plt.title(
            f"{software} {testmode}最佳并行模式{best_mode}资源使用情况（GPU）（全程{time}s）",
            fontproperties=my_font,
            fontsize=10,
            y=1.05,
        )
...
def hpc_resource_radar_split(
    ...
    if datatype[1] == "singletask":
        ...
        plt.title(
            f"{software} {testmode}最佳并行模式{best_mode}资源使用情况（分层级）（全程{time}s）",
            fontproperties=my_font,
            fontsize=10,
            y=1.05,
        )
...
def extract_and_draw(
    mondir, bins, cpus, gpus, outputfile, gpu_outputfile, split_outputfile, datatype
):
    cpu_data, power_data, mem_data, read_data, write_data, gpu_data, gpu_mem_data = (
        np.array([]),
        np.array([]),
        np.array([]),
        np.array([]),
    ...
    for i in range(len(bins)):
        ...
        cpu_data = np.append(cpu_data, extract_cpu(f"{mondir}/cpu_{i}.json", cpus))
        power_data = np.append(power_data, extract_power(f"{mondir}/power_{i}.json"))
        mem_data = np.append(
            mem_data,
            extract_mem(
                f"{mondir}/memFree_{i}.json",
                f"{mondir}/memCached_{i}.json",
                f"{mondir}/memBuffers_{i}.json",
                f"{mondir}/memTotal.json",
            ),
        )
        read_data = np.append(read_data, extract_read(f"{mondir}/read_{i}.json"))
        write_data = np.append(write_data, extract_write(f"{mondir}/write_{i}.json"))
        gpu_data = np.append(
            gpu_data, extract_gpu(f"{mondir}/gpu_{i}.json", gpus, start, end)
        )
        gpu_mem_data = np.append(
            gpu_mem_data, extract_gpu_mem(f"{mondir}/gpuMem_{i}.json", gpus, start, end)
        )
...
if __name__ == "__main__":
    extract_and_draw(
        "tmp/",
        [(1736416077, 1736427076), (1736427077, 1736436076)],
        32,
        8,
        "test.png",
        "test_gpu.png",
